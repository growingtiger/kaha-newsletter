...
// ═══ 1. 수술등중대진료(마취) 동의서 ════════════════════════════════
{
  const body = [].concat(
    titleBlock("수술등중대진료(마취) 동의서",
      "본 동의서는 「수의사법」 제13조의2 및 같은 법 시행규칙 제13조의3에 따라 작성되었습니다.", 40),
    [
      infoTable([
      ...
      sec("설명 및\n동의사항", [
        new (require("docx").Table)({
          columnWidths: [IW - mm(46), mm(46)],
          width: { size: IW, type: require("docx").WidthType.DXA },
          rows: [new (require("docx").TableRow)({
            children: [
              plainCell(bulletLines([
                "위 반려동물의 수술등중대진료를 위해 전신마취가 필요합니다.",
                "마취 중 심정지가 발생할 경우 아래 처치에 동의합니다.",
              ]), { w: IW - mm(46), margins: { top: 0, bottom: 0, left: 0, right: 0 } }),
              plainCell(bulletLines(["심폐소생술 원함", "심폐소생술 원하지 않음"], { marker: "□ " }),
                { w: mm(46), margins: { top: 0, bottom: 0, left: 0, right: 0 } }),
            ],
          })],
        }),
      ], { note: "동의하시는 부분에\n체크해주세요.", height: mm(18) }),
      ...
      sec("수술등중대진료의\n필요성 ·\n방법 및 내용", [
        ...
        ], [mm(22), Math.floor((IW - mm(42)) / 2), mm(20), IW - mm(42) - Math.floor((IW - mm(42)) / 2)],
          [mm(6.5), mm(6.5), mm(8), mm(8)]),
      ], { height: mm(31.5) }),
      gap(150),
      sec("수술등중대진료에\n따라 전형적으로\n발생이 예상되는\n후유증 또는 부작용", [
        fieldTable([
      ...
      sec("수술등중대진료\n전후에 보호자의\n준수 및 숙지사항",
        ...
        ], { size: 15, lineH: 250 }), { height: mm(30) }),
      gap(220),
      table([TOTAL], [[plainCell(closing(
        "「수의사법」 제13조의2 및 같은 법 시행규칙 제13조의3에 따라 위와 같이 수의사로부터 수술등중대진료의 필요성 · 방법 · 내용과 전형적으로 발생이 예상되는 후유증 및 부작용에 관한 설명을 충분히 듣고 이해하였습니다. 이에 위 수술등중대진료 및 마취의 시행에 동의하며, 시행 과정에서 필요한 수의학적 판단과 처리를 담당 수의사에게 위임합니다.",
        "*기타 내용이 발생할 경우 별지를 포함합니다."),
        { w: TOTAL, margins: { top: 0, bottom: 0, left: 0, right: 0 } })]], mm(19)),
      gap(180),
      signRow(["보호자", "설명 수의사"]),
    ]);
...
// ═══ 2. 입원 동의서 ════════════════════════════════════════════════
{
  ...
  const body = [].concat(
    ...
    [
      ...
      ], { size: 15, lineH: 260 }), { height: mm(17) }),
      gap(190),
      sec("응급 상황 및\n사망 시 처리", bulletLines([
      ...
      ], { size: 15, lineH: 260 }), { height: mm(14) }),
      gap(220),
      table([TOTAL], [[plainCell(closing(
...
// ═══ 3. 퇴원 안내문 ════════════════════════════════════════════════
{
  ...
  const body = [].concat(
    ...
    [
      ...
      ], [lw, IW - lw - mm(20) - mm(26), mm(20), mm(26)], mm(8))], { height: mm(17.5) }),
      gap(190),
      sec("가정 투약 안내", [
      ...
      ], [rw + mm(4), rv, rw, IW - rw * 2 - rv - mm(4)], [mm(8), mm(12), mm(8)])],
        { height: mm(31.5) }),
      gap(220),
      table([TOTAL], [[plainCell(closing(
...
// ═══ 4. 마취 전 평가 · 모니터링 체크리스트 ═════════════════════════
{
  ...
  const body = [].concat(
    ...
    [
      infoTable([
        ["반려동물", [["이름 / 차트번호", "", "종 / 품종", ""], ["성별 / 중성화", "", "연령 / 체중", ""]]],
        ["수의사", [["동물병원명", "", "담당 수의사", ""]]],
      ], IN_W, 392),
      gap(170),
      sec("환자 평가", [
...
// ═══ 5. 영양 평가 · BCS / MCS 기록지 ═══════════════════════════════
{
  ...
  const body = [].concat(
    ...
    [
      ...
      ], [lw3, hv, IW - lw3 - hv], [mm(9), mm(15), mm(15), mm(26)])], { height: mm(65) }),
      gap(240),
      signRow(["평가자"]),
    ]);
...
Promise.all(jobs).then(() => console.log("워드 양식 5종 생성 완료"))
