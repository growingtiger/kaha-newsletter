...
// ═══ 7. 사진·영상 촬영 및 SNS·홍보 활용 동의서 (보호자용) ══════════════
{
  const lw = mm(26);
  const body = [].concat(
    titleBlock("사진·영상 촬영 및 SNS·홍보 활용 동의서",
      "본 동의서는 진료 사진·영상을 병원 홍보 및 SNS 게시에 활용하기에 앞서 보호자의 동의를 확인하기 위해 작성되었습니다.", 32),
    [
      infoTable([
        ["보호자", [["성명", "", "연락처", ""]]],
        ["반려동물", [["이름", "", "종 / 품종", ""]]],
      ...
      sec("비공개 · 삭제\n요청", [].concat(
        ...
        [para("", { size: 8 })],
        [fieldTable([
          [{ text: "제외 요청 항목", label: true }, ""],
        ], [lw, IW - lw], [mm(9)])],
      ), { height: mm(28) }),
      gap(170),
      sec("동의 철회", bulletLines([
        "위 동의는 언제든지 병원에 연락하여 철회할 수 있으며, 철회 후에는 새로운 게시에 사용하지 않습니다.",
        "본 동의는 진료 자체와는 무관하므로, 동의하지 않아도 진료를 받는 데 불이익이 없습니다.",
      ], { size: 15, lineH: 260 }), { height: mm(20) }),
      gap(220),
      table([TOTAL], [[plainCell(closing(
        "위와 같이 진료 사진 · 영상의 촬영 목적과 활용 범위, 비공개 · 삭제 요청 및 동의 철회에 관하여 안내받았으며, 이를 이해하고 위 내용에 동의합니다."),
        { w: TOTAL, margins: { top: 0, bottom: 0, left: 0, right: 0 } })]], mm(15)),
      gap(180),
      signRow(["보호자"]),
    ]);
  jobs.push(buildDoc("사진·영상 촬영 및 SNS·홍보 활용 동의서", body, "04_병원운영/사진영상촬영SNS활용동의서_한국동물병원협회.docx"));
}

Promise.all(jobs).then(() => console.log("워드 양식 7종 생성 완료"))
