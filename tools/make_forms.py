...
s = title_block("수술등중대진료(마취) 동의서",
                "본 동의서는 「수의사법」 제13조의2 및 같은 법 시행규칙 제13조의2에 따라 작성되었습니다.",
                title_size=22)
s.append(info_table([
...
s.append(sec("설명 및\n동의사항", stack([
    ...
    Spacer(1, 1.6 * mm),
    bullets(["심폐소생술 원함&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;□ 심폐소생술 원하지 않음"], marker="□ ", indent=0),
], IW), note="동의하시는 부분에\n체크해주세요.", pad_top=5))
s.append(gap())
...
s.append(sec("수술등중대진료의\n필요성 ·\n방법 및 내용", fields([
    ...
    [P("필요성"), "", None, None],
    [P("방법 및 내용"), "", None, None],
], [lw, vw, mw, vw], [7.2 * mm, 7.2 * mm, 8 * mm, 8 * mm], valign_top=(2, 3)),
    pad_top=3, pad_left=5))
s.append(gap())

s.append(sec("수술등중대진료에\n따라 전형적으로\n발생이 예상되는\n후유증 또는 부작용", stack([
    grid_list(["저혈압, 저체온, 고체온", "약물과민반응에 의한 쇼크", "순환부전에 의한 쇼크",
               "췌장염, 신부전 및 폐수종", "기저질환 발현 및 현증악화", "삽관에 의한 일시적 기침"],
              IW, cols=3, size=8.6, leading=11.6),
    Spacer(1, 1.8 * mm),
    asa_table(IW, row_h=4.7 * mm),
], IW), pad_top=4))
s.append(gap())
...
], size=8.4, leading=11.8), pad_top=3))
s.append(Spacer(1, 2.6 * mm))
s.append(closing(
    "「수의사법」 제13조의2 및 같은 법 시행규칙 제13조의2에 따라 위와 같이 수의사로부터 수술등중대진료의 필요성 · 방법 · "
    "내용과 전형적으로 예상되는 후유증 및 부작용에 관한 설명을 충분히 듣고 이해하였습니다. 이에 위 수술등중대진료 및 마취의 "
    "시행에 동의하며, 시행 과정에서 필요한 수의학적 판단과 처리를 담당 수의사에게 위임합니다."))
s.append(Spacer(1, 2.6 * mm))
...
], [lw, vw, lw, vw], [9.4 * mm] * 3), pad_top=5, pad_left=5))
s.append(gap())
...
s.append(sec("평가 결과 및 계획", fields([
    ...
    [9 * mm, 14 * mm, 14 * mm, 22 * mm], valign_top=(1, 2, 3)), pad_top=5, pad_left=5))
s.append(Spacer(1, 4 * mm))
...
# ═══════════════════════════════════════════════════════════════════
G[0] = 6 * mm
set_form("개인정보 수집·이용 동의서")
s = title_block("개인정보 수집·이용 동의서",
                "본 동의서는 「개인정보 보호법」에 따라 진료 예약·안내 등에 필요한 보호자 개인정보의 수집·이용에 대한 동의를 확인하기 위해 작성되었습니다.",
                title_size=20)
s.append(info_table([
    ("보호자", [[P("성명"), "", P("연락처"), ""], [P("주소"), "", None, None]]),
    ("반려동물", [[P("이름"), "", P("종 / 품종"), ""]]),
    ("수의사", [[P("동물병원명"), "", None, None]]),
], widths=IN_W, heights=[10 * mm] * 4))
s.append(gap())
...
s.append(sec("수집·이용\n목적", bullets([
    ...
    "응급 상황 발생 시 보호자 연락",
], size=8.8, leading=14), pad_top=6))
s.append(gap())

lw = 26 * mm
s.append(sec("수집 항목", fields([
    [P("필수 항목"), P("성명, 연락처, 주소, 반려동물 정보(이름 · 종 / 품종)")],
    [P("선택 항목"), P("이메일 주소 (검사 결과지 · 안내자료 전송 시)")],
], [lw, IW - lw], [10 * mm, 10 * mm]), pad_top=6, pad_left=5))
s.append(gap())
...
], size=8.4, leading=13.4), pad_top=6))
s.append(gap())

s.append(sec("동의 거부 권리 및\n열람 · 정정 요청", bullets([
...
s.append(Spacer(1, 4 * mm))
s.append(closing(
    "위와 같이 개인정보의 수집 · 이용 목적, 항목, 보유 및 이용 기간과 동의 거부 권리에 관하여 안내받았으며, "
    "이를 이해하고 위 내용에 따른 개인정보 수집 · 이용에 동의합니다."))
s.append(Spacer(1, 3 * mm))
s.append(sign_row(["보호자"]))
build("04_병원운영/개인정보수집이용동의서_한국동물병원협회.pdf", "개인정보 수집·이용 동의서", s)

# ═══════════════════════════════════════════════════════════════════
# 7. 사진·영상 촬영 및 SNS·홍보 활용 동의서 (보호자용)
# ═══════════════════════════════════════════════════════════════════
G[0] = 6 * mm
set_form("사진·영상 촬영 및 SNS·홍보 활용 동의서")
s = title_block("사진·영상 촬영 및 SNS·홍보 활용 동의서",
                "본 동의서는 진료 사진·영상을 병원 홍보 및 SNS 게시에 활용하기에 앞서 보호자의 동의를 확인하기 위해 작성되었습니다.",
                title_size=19)
s.append(info_table([
    ("보호자", [[P("성명"), "", P("연락처"), ""]]),
    ("반려동물", [[P("이름"), "", P("종 / 품종"), ""]]),
    ("수의사", [[P("동물병원명"), "", None, None]]),
], widths=IN_W, heights=[10 * mm] * 3))
s.append(gap())

s.append(sec("촬영·활용\n목적", bullets([
    "진료 전후 경과 기록 및 원내 진료 참고 자료로 활용",
    "병원 공식 SNS(인스타그램 등) 및 홈페이지 게시",
    "원내 게시판 · 대기실 화면 등 병원 내 안내 자료로 활용",
], size=8.8, leading=14), pad_top=6))
s.append(gap())

s.append(sec("게시 범위", bullets([
    "진료 사진 · 영상의 병원 SNS · 홈페이지 게시                    □ 동의   □ 동의하지 않음",
    "게시 시 반려동물 이름 표기                                     □ 동의   □ 비공개 요청",
    "보호자 등 사람이 함께 촬영된 사진 · 영상의 게시                  □ 동의   □ 동의하지 않음",
], size=8.6, leading=14), pad_top=6))
s.append(gap())

s.append(sec("비공개 · 삭제\n요청", stack([
    bullets([
        "이미 게시된 사진 · 영상도 보호자가 요청하면 지체 없이 삭제합니다.",
        "특정 사진 · 영상만 게시에서 제외하고 싶은 경우 아래에 적어 주십시오.",
    ], size=8.8, leading=14),
    Spacer(1, 2 * mm),
    fields([[P("제외 요청 항목"), ""]], [26 * mm, IW - 26 * mm], [9 * mm]),
], IW), pad_top=6, pad_left=5))
s.append(gap())

s.append(sec("동의 철회", bullets([
    "위 동의는 언제든지 병원에 연락하여 철회할 수 있으며, 철회 후에는 새로운 게시에 사용하지 않습니다.",
    "본 동의는 진료 자체와는 무관하므로, 동의하지 않아도 진료를 받는 데 불이익이 없습니다.",
], size=8.8, leading=14), pad_top=6))
s.append(Spacer(1, 4 * mm))
s.append(closing(
    "위와 같이 진료 사진 · 영상의 촬영 목적과 활용 범위, 비공개 · 삭제 요청 및 동의 철회에 관하여 안내받았으며, "
    "이를 이해하고 위 내용에 동의합니다."))
s.append(Spacer(1, 3 * mm))
s.append(sign_row(["보호자"]))
build("04_병원운영/사진영상촬영SNS활용동의서_한국동물병원협회.pdf", "사진·영상 촬영 및 SNS·홍보 활용 동의서", s)
